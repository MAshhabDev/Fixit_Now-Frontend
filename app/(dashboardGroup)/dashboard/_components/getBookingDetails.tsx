          ...
          <div className="space-y-5 text-xs">
            ...
            <div className="bg-accent/40 border border-border p-4 rounded-2xl flex items-center justify-between">
              <div>
                ...
                <span className={`inline-block px-2.5 py-0.5 mt-1 rounded-full text-[10px] font-extrabold uppercase border border-border ${
              ...
              <div className="text-right">
                ...
                <p className="text-xl font-black text-emerald-600 flex items-center justify-end">
                  <DollarSign className="w-4 h-4" /> ৳{data?.totalAmount || service?.price || 0}
                </p>
              </div>
            </div>
            ...
            <div className="space-y-2 border-t border-border pt-3">
              <div className="flex items-center gap-4 text-muted-foreground">
                <span className="flex items-center gap-1">
                  <Calendar className="w-3.5 h-3.5 text-primary" /> Date: {data?.bookingDate || "N/A"}
                </span>
                <span className="flex items-center gap-1">
                  <Clock className="w-3.5 h-3.5 text-primary" /> Slot: {data?.timeSlot || "N/A"}
                </span>
              </div>

              <div className="flex items-start gap-1 text-muted-foreground pt-1">
                <MapPin className="w-3.5 h-3.5 text-primary shrink-0 mt-0.5" />
                <span>Address: {data?.serviceAddress || "Dhaka, Bangladesh"}</span>
              </div>
            </div>
